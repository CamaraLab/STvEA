...
```{r include=FALSE}
# CITE-seq protein
cite_protein <- read.table("/home/rstudio/Dropbox/Projects/PGC001_Heterotypic_signaling/Analyses/PGC001.a15/data/protein_expr_all.csv", sep=",", row.names=1, header=TRUE, stringsAsFactors = FALSE)
cite_protein <- as.data.frame(t(cite_protein))
colnames(cite_protein) <- sapply(colnames(cite_protein), function(x) gsub("ADT_", "", x))

# CITE-seq mRNA
cite_mRNA <- read.table("/home/rstudio/Dropbox/Projects/PGC001_Heterotypic_signaling/Data/CITE-seq/balbc_all/gene_matrix_all_new.csv",row.names=1, header = TRUE, sep=",", stringsAsFactors = FALSE)
genes <- read.table("/home/rstudio/Dropbox/Projects/PGC001_Heterotypic_signaling/Analyses/PGC001.a15/data/genes.tsv", row.names=1, header=FALSE, sep="\t", stringsAsFactors = FALSE)
...
# CITE-seq extra
cite_latent <- read.table("/home/rstudio/Dropbox/Projects/PGC001_Heterotypic_signaling/Analyses/PGC001.a15/data/latent_output_v2.txt", sep=",", header=FALSE, row.names=1, stringsAsFactors = F)
```
```{r}
...
We cluster the CITE-seq cells based on their mRNA expression using a consensus of HDBSCAN clusterings.
First, we run UMAP on the CITE-seq latent space to the same number of dimensions (running UMAP before density based clustering is suggested in the UMAP documentation https://umap-learn.readthedocs.io/en/latest/clustering.html). We perform a scan over the two parameters of the Python HDBSCAN implementation, min_cluster_size and min_samples (more information on selecting these paramters https://hdbscan.readthedocs.io/en/latest/parameter_selection.html).
```{r}
